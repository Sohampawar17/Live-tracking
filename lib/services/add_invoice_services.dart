...
class AddInvoiceServices{
  ...
  Future<bool> updateOrder(AddInvoiceModel orderdetails) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Fluttertoast.showToast(msg: "Invoice Updated successfully");
        return true;
      } else {
  ...
  Future<bool> addOrder(AddInvoiceModel orderdetails) async {
    baseurl =  await geturl();
  ...
  Future<List<InvoiceItems>> fetchitems() async {
    baseurl =  await geturl();
    try {
      var dio = Dio();
      var response = await dio.request(
